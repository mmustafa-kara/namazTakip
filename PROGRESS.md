...
- [ ] Tekrar kullanılabilir UI bileşenlerinin (Özel butonlar, Flat Premium Prayer Card, Timer, BottomNavigationBar) kodlanması.
...
- [ ] Ana sayfa UI tasarımının koda dökülmesi.
- [ ] Günlük 6 vaktin (İmsak, Güneş, Öğle, İkindi, Akşam, Yatsı) listeleneceği yatay/dikey kartların oluşturulması.
- [ ] O anki vaktin hesaplanıp UI üzerinde vurgulanması (Highlight).
- [ ] Bir sonraki vakte kalan süreyi hesaplayan ve saniye saniye güncellenen dinamik geri sayım sayacının (Timer) ViewModel içerisine yazılması.

## 🧭 FAZ 5: Sensörler ve Kıble Pusulası Modülü
- [ ] Pusula sekmesinin arayüzünün (karanlık, minimalist kadran) kodlanması.
- [x] `flutter_compass` paketi ile manyetometre ve jiroskop verilerinin dinlenmesi.
- [x] Cihazın konumuna göre Kabe'nin açısının hesaplanması ve ibrenin bu açıya göre animasyonlu döndürülmesi (Arka plan mantığı).
- [ ] Hedef açıya ulaşıldığında Haptic Feedback (titreşim) tetiklenmesi.

## 🔔 FAZ 6: Arka Plan İşlemleri ve Bildirimler
- [ ] `flutter_local_notifications` paketinin temel konfigürasyonunun (Android/iOS izinleri) yapılması.
- [ ] Veritabanındaki namaz vakitlerine göre ileri tarihli zamanlanmış bildirimlerin (Scheduled Notifications) ayarlanması.
- [ ] Ayarlar sekmesi UI'ının yapılması (Bildirim sesi seçimi, kerahet vakti uyarısı, konum güncelleme tercihleri).

## 📿 FAZ 7: Zikir ve Kaza Takip (Kişisel Modül)
- [ ] Zikirmatik sayfasının UI kodlaması (Büyük dairesel interaktif buton, resetleme seçenekleri).
- [ ] Kaza namazı ve orucu takip listesinin UI kodlaması (+/- butonları ile interaktif liste).
- [ ] Zikir ve kaza sayılarının yerel veritabanına (Hive) anlık kaydedilmesi ve okunması.
